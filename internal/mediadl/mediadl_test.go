...
func TestIsStatusCodeNetworkBlocked(t *testing.T) {
	...
	if !isStatusCodeNetworkBlocked(errors.New("unexpected status code: 403")) {
		t.Fatal("expected forbidden status to be treated as network blocked")
	}
	if !isStatusCodeRateLimited(errors.New("unexpected status code: 429")) {
		t.Fatal("expected text status to be treated as rate limited")
	}
	if isStatusCodeNetworkBlocked(errors.New("unexpected status code: 429")) {
		t.Fatal("did not expect 429 to be treated as generic network blocked")
	}
	if isStatusCodeNetworkBlocked(errors.New("unexpected status code: 404")) {
