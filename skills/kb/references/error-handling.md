...
| YouTube caption extraction fails before transcript ingest | The preferred `yt-dlp` backend is missing, outdated, or failed, and the legacy extractor could not recover | Install or update `yt-dlp`, or set `[youtube].yt_dlp_path` / `YOUTUBE_YT_DLP_PATH` to the intended executable |
| YouTube `network_blocked` / blocked captions or audio | YouTube returned 400/403/429/5xx after backend selection, often due to bot detection, authentication, rate limiting, or datacenter IP blocking | After confirming `yt-dlp` is installed and current, configure `[youtube].proxy`, `[youtube].cookies_file`, or run from a trusted/residential network |
