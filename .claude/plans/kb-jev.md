...
| `state.jsonl` | `{path, body_hash, contract, banks:{bank_id: version}, written:{key: value_hash}, bank_body:{bank_id: body_hash}, bank_contract:{bank_id: contract}, written_body:{key: body_hash}, facts:{name: value}, updated}`; `body_hash`/`contract` = last write (rename detection), freshness is per bank/key (`JudgedBody`, `JudgedContract`, `KeyBody`; legacy rows fall back to `body_hash`/`contract`) | corpus |
...
- **Bank ids recorded in state rows** (`StateMeta.Banks`): sources `{relevance, quality, classify, concept}`, articles `{classify, concept}`; link adds `link` and `link_candidates` (hash of sorted candidate ids). Lint's `unclassified` compares these with `questions.Load(id).Version`. Each bank in `StateMeta.Banks` is stamped with the body the write started from (`bank_body`) and the contract (`bank_contract`); other writes never advance those stamps, except a body edit that only adds wikilink markup (link insertion), which carries every stamp forward. Undecided classify/link runs record blank versions. `StateMeta.Facts` holds per-document outcomes (classify: `primary_concept: none|concept`, counted across the topic for concept proposals).
